...
SUPPORTED_BROWSERS = ['chrome', 'firefox', 'RemoteChromeDriver', 'sauce_desktop']
...
def pytest_bdd_before_scenario(request, feature, scenario):
    ...
    # print("current scenario_name is :",scenario_name)
    global manualexecutiontime
    pattern = r'\{(\d+)\}'
    matches = re.findall(pattern, scenario.name)
    # If matches are found, extract the manualexecutiontime and scenario name
    if matches:
    ...
    def check_texts(self, driver):
        ...
        for text in self.target_texts:
            if text in page_source:
                # print("Text '{}' has been encountered.".format(text))
                cfv = True
                # print("Cloudflare has been encountered")
        if cfv:
    ...
    for report in pytest_report:
        if report.when == 'call':
            ...
            try:
                ...
                # driver.execute_script(f"sauce:job-build=CF{site_str}")
                try:
                    driver.execute_script("sauce:job-build=CF_Failure_Android")
                    driver.execute_script(f"sauce:job-result={report.outcome.lower()}")
                    driver.execute_script(f"sauce:job-name={scenario_name} for {site_str}")
                except Exception as e:
                    print(f"Note: SauceLabs commands not available in local browser: {e}")
            finally:
                if driver:
                    try:
                        driver.quit()
...
def browser(config_browser, config_wait_time, request, site, rversion):
    ...
    '''# current_feature = request.config.getoption('--current-feature')'''
    current_scenario = scenario
    # print("current_feature: ",current_feature)
    print("current_scenario 'before yield driver': ", current_scenario)
    # TODO
    # def delete_cache
    """ Can be used to work with cloud file upload:
    # def fileupload(driver):
    # driver.file_detector = LocalFileDetector()
    # file_input = driver.find_element(By.CSS_SELECTOR, "(//input[@id='FileUpload121'])[1]")
