...
def get_xpath_index(config_browser):
    ...
    """
    if any(cb in config_browser for cb in [
        'sauce_android',
        'sauce_android_tunnel',
        'sauce_iphone',
        'sauce_ipad'
    ]):
        xpath_index = 2
...
def get_locator_type_and_value(element_name, locator_dict):
    ...
    for locator_type, elements in locator_dict.items():
        if element_name in elements:
            ...
            if by_type:
                print(
                    f" '{element_name}' is found using the locator type '{by_type_raw}' "
                    f"and the value is '{locator_value}'"
                )
...
def hover_mouse_over(element):
    ...
    The parameter can be either a WebElement or a locator*.
    """
...
def scroll_top_to_bottom():
    ...
    def is_bottom():
        driver.execute_script("""
        window.isReachedBottom = function() {
            return (window.innerHeight + window.scrollY) >= document.body.offsetHeight;
        }
        """)
        return driver.execute_script("return window.isReachedBottom();")
...
def wait_and_click(locator, time_in_sec=10):
    (WebDriverWait(driver, time_in_sec).until(
        EC.element_to_be_clickable(locator)
    )).click()
